...
def rooms(request):
    ...
    if request.method == "POST":
        ...
        if "filter" in request.POST:
            ...
            return render(request, path + "guest/rooms.html", context)

    context = {"role": role, 'rooms': rooms, 'fd': firstDayStr, 'ld': lastDateStr}
    return render(request, path + "guest/rooms.html", context)
...
def add_room(request):
    ...
    context = {"role": role}
    return render(request, path + "admin/add-room.html", context)
...
def room_profile(request, id):
    ...
    return render(request, path + "guest/room-profile.html", context)
...
def room_edit(request, pk):
    ...
    return render(request, path + "admin/room-edit.html", context)
...
def current_room_services(request):
    ...
    if curBooking is not None:
        ...
        context = {"role": role, "error": "You Don't Have Booking Right Now"}
        return render(request, path + "guest/current-room-services.html", context)
...
def bookings(request):
    ...
    if request.method == "POST":
        if "filter" in request.POST:
            if request.POST.get("number") != "":
                rooms = Room.objects.filter(number__contains=request.POST.get("number"))
    ...
    return render(request, path + "admin/bookings.html", context)
...
def booking_make(request):
            ...
            for i in range(room.capacity - 1):
                ...
                if request.POST.get(nameid) != "":
                    if request.POST.get(nameid) != None:
                        d = Dependees(booking=curbooking, name=request.POST.get(nameid))
                        d.save()
    ...
    return render(request, path + "guest/booking-make.html", context)
...
def deleteBooking(request, pk):
    ...
    return render(request, path + "admin/deleteBooking.html", context)
...
def refunds(request):
    ...
    context = {"role": role, 'refunds': refunds}
    ...
    if request.method == "POST":
        if "decline" in request.POST or "approve" in request.POST:
            ...
            def send(request, receiver, text, subject):
                ...
                return render(request, path + "admin/refunds.html", context)
        ...
        if "filter" in request.POST:
            ...
            if request.POST.get("gid") != "":
                users = users.filter(id__contains=request.POST.get("gid"))
                guests = Guest.objects.filter(user__in=users)
            ...
            if request.POST.get("reason") != "":
                refunds = refunds.filter(reason__contains=request.POST.get("reason"))
    ...
    if request.method == "POST":
        if "sendReq" in request.POST:
            ...
            if not temp:
                ...
                currentReq.save()
                messages.success(request, "Your request was successfully sent.")
            else:
                messages.error(
                    request, "We already have your refund request for this reservation!"
                )

    context = {"role": role, "curGuest": curGuest, "id": request.POST.get("bookingId")}

    return render(request, path + "guest/request-refund.html", context)
